...
newtype CS r l m a = CS
  { rCS :: (a -> l m r -> m r) -> l m r -> m r
  }
...
newtype ExtB l e t m r = ExtB
  { rExtB :: (t (CS r l m) e -> l m r -> m r, l m r)
  }

class (Monad b, Monad (t b), MonadTrans t) =>
      MonadL t b
  where
...
lreflect t =
  CS $ \k (ExtB (mk, s)) ->
    mk (t >>= \a -> glue (CS $ \k' s' -> k a (ExtB (k', s')))) s

lreify :: MonadL t (CS r l m) => CS r (ExtB l a t) m a -> t (CS r l m) a
lreify t =
  glue
    (CS $ \k s -> rCS t (\a (ExtB (mk, s')) -> mk (return a) s') (ExtB (k, s)))
...
superL ::
     (forall a b. CS r l m a -> CS r l m b)
...
join'inc m =
  CS $ \k'' (ExtB (n, s)) -> rCS m (\x s' -> rCS x k'' (ExtB (n, s'))) s
