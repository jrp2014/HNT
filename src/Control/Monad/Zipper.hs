...
zip'cont :: CS r (ExtB l e (StateT (Z.Zipper term dir m1))) m
              (Z.ZipReq term dir -> m1 (Z.Zipper term dir m1))
zip'cont = getsL $ Z.zip'cont

zip'update :: term -> CS r (ExtB l e (StateT (Z.Zipper term dir (CS r l m)))) m ()
zip'update t = mmodifyL $ Z.zip'update  t

zip'isRoot :: (Monad m1) => CS r (ExtB l e (StateT (Z.Zipper term dir m1))) m Bool
zip'isRoot = getsL $ Z.zip'isRoot

zip'move :: Z.Dir dir -> CS r (ExtB l e (StateT (Z.Zipper term dir (CS r l m)))) m ()
zip'move d = mmodifyL $ Z.zip'move d

zip'moves :: [Z.Dir dir] -> CS r (ExtB l e (StateT (Z.Zipper term dir (CS r l m)))) m ()
...
runZipL :: (Z.ZipperAble term dir) =>
           term
           -> CS r1 (ExtB l1 a (StateT (Z.Zipper term dir (CS r l m)))) m a
           -> CS r1 l1 m a
runZipL t m = runStateL (Z.zipper t) m >>= return . fst
