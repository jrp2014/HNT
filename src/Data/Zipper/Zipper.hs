...
data ZipReq term dir
  = Move (Dir dir) -- ^ Move the zipper to dir
...
instance (Show dir, Show term) => Show (Zipper term dir m) where
  show (Zipper p t _) =
    "Zipper " ++ " (term = " ++ show t ++ ") (path = " ++ showPath p ++ ")"
...
zip'moves ::
     (Monad m) => [Dir dir] -> Zipper term dir m -> m (Zipper term dir m)
...
--        : Head of Stack of Possdir
  ...
  zipper t =
    Zipper
      ...
      t
      (\req -> do
         z <- runContL $ runStackL $ runStackL $ runStackL (fix (loop []) t)
         zip'cont z req)
    where
      fix f s = do
        putL $ possibleDirs s
      ...
      loop p s = do
        ...
        case req of
          ...
          Move Next -> do
            ...
            loop p s
      goIn d p u =
        case lookup d (possibleDirs u) of
          Nothing -> return u
          Just (s, f) -> do
            newpossdir <- getsL $ filter (\(x, _) -> x /= d)
            oldstack <- inc getL
      ...
      findOneDown ((d, _):_) = do
        inc2 $ appendStateL (DownTo d)
        return True
