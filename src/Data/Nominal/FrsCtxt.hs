...
newtype FrsCtxt v a = FrsCtxt
  { unFrsCtxt :: Map.Map v (Set.Set a)
  } deriving (Eq, Ord)
...
instance (Show v, Show a) => Show (FrsCtxt v a) where
  show (FrsCtxt m) =
    printSeq "" "" " " $
    map (\(v, s) -> showAtmSet s ++ "#v:" ++ show v) (Map.toList m)

showAtmSet :: (Show a) => Set.Set a -> String
showAtmSet s = printSeq "{" "}" " " (map (\a -> "a:" ++ show a) (Set.elems s))

parse'constr ::
     (ParsecRead a, ParsecRead b, Ord b) => GenParser Char st (a, Set.Set b)
parse'constr =
  (do s <- parse'set (string "a:" >> parsecRead)
      spaces >> char '#' >> spaces
...
parse'frsCtxt ::
     (Ord b, Ord a, ParsecRead a, ParsecRead b)
  => GenParser Char st (FrsCtxt a b)
parse'frsCtxt = do
  l <- parse'seq parse'constr
  return $ FrsCtxt $ foldl f Map.empty l
  where
    f m (v, s) = Map.insertWith Set.union v s m

instance (Ord atm, Ord var, ParsecRead atm, ParsecRead var) =>
         ParsecRead (FrsCtxt var atm) where
...
mergeFrsCtxt (FrsCtxt fc1) (FrsCtxt fc2) =
  FrsCtxt $ Map.unionWith Set.union fc1 fc2
...
isSubfrsCtxtOf (FrsCtxt fc1) (FrsCtxt fc2) =
  Map.isSubmapOfBy Set.isSubsetOf fc1 fc2
