-- | Nominal Terms
module Data.Nominal.Term.Term where
...
data Term atm cst var
  = Leaf (Leaf atm cst var) -- ^ a leaf
  | Pair (Term atm cst var)
         (Term atm cst var) -- ^ a pair
  | Modal (Modal atm)
          (Term atm cst var) -- ^ a modal
  deriving (Eq, Ord)
...
parse'leaf ::
     (ParsecRead atm, ParsecRead cst, ParsecRead var)
  => GenParser Char st (Leaf atm cst var)
parse'leaf = (parse'atm <|> parse'var <|> parse'cst) <?> "leaf"

instance (ParsecRead atm, ParsecRead cst, ParsecRead var) =>
         ParsecRead (Leaf atm cst var) where
...
parse'term ::
     (ParsecRead atm, ParsecRead cst, ParsecRead var)
  => GenParser Char st (Term atm cst var)
parse'term = (Leaf <$> parse'leaf) <|> try parse'tmodal <|> parse'pair

instance (ParsecRead atm, ParsecRead cst, ParsecRead var) =>
         ParsecRead (Term atm cst var) where
...
substituteM ::
     (Monad m)
  => (var -> m (Maybe (Term atm cst var)))
  -> Term atm cst var
  -> m (Term atm cst var)
substituteM f = substM'
  where
    f' v = fromMaybe (var v) <$> f v
...
substitute ::
     (var -> Maybe (Term atm cst var)) -> Term atm cst var -> Term atm cst var
