...
data Rule atm cst var = Rule
  { rule'ctxt :: FC.FrsCtxt var atm -- ^ context of the rule
...
parse'rule ::
     (Ord var, Ord atm, ParsecRead atm, ParsecRead cst, ParsecRead var)
  => GenParser Char st (Rule atm cst var)
parse'rule =
  (do mc <-
        optionMaybe
          (do c <- FC.parse'frsCtxt
...
instance (Ord atm, Ord var, ParsecRead atm, ParsecRead cst, ParsecRead var) =>
         ParsecRead (Rule atm cst var) where
  parsecRead = parse'rule

instance (Ord atm, Ord var, ParsecRead atm, ParsecRead cst, ParsecRead var) =>
         ParsecRead (CtxtTerm atm cst var) where
  parsecRead = parse'ctxtTerm

-- * Rewriting Functions
rewrite ::
     (Ord t1, Ord t, Show t2, Eq t2)
  => Rule t1 t2 t
  -> Term t1 t2 t
  -> CS r (ExtB (ExtB l e1 (ExceptT String)) e (StateT (FC.FrsCtxt t t1))) m (Term t1 t2 t)
rewrite (Rule fc l r) t = do
  frs <- getL
