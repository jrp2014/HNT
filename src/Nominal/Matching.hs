...
checkAtom ::
  ...
  -> CS
       r
       (ExtB
          (ExtB (ExtB l e11 (ExceptT String)) e1 t1) e (StateT (E.Env a)))
       m
       a
...
checkTerm ::
  ...
  (var
   -> CS
        r
        (ExtB
           (ExtB (ExtB l e (ExceptT String)) e1 t1) e11 (StateT (E.Env a)))
        m
        t)
  -> Term a cst var
  -> CS
...
core ::
  ...
  (t1
   ...
   -> CS
        r
        (ExtB
           (ExtB (ExtB l e11 (ExceptT String)) e1 t11) e (StateT (E.Env a)))
        m
        ())
  -> (t1
      -> CS
...
core fsubst frsConst = core'
  where
    ...
    core' t@(Leaf (Var     v))                  u    = if t == u
                                                        then (do p <- perm
                                                                 localSetFreshS
                                                                  (Set.map (True,) $ P.supp p)
                                                                  (frsConst v)
                                                             )
                                                        else fsubst v u
...
core'run ::
  ...
  (t
   ...
   -> CS
        r
        (ExtB
           (ExtB (ExtB l e (ExceptT String)) e1 t11) () (StateT (E.Env a)))
        m
        ())
  -> (t
      -> CS
...
checkSatisfied ::
  ...
  -> CS
       r
       (ExtB
          (ExtB (ExtB l e11 (ExceptT String)) e1 (StateT (FC.FrsCtxt v a)))
          e
          (StateT (E.Env a)))
       m
       ()
...
fsubst'error ::
  t
  -> t1
  -> CS r (ExtB (ExtB (ExtB l e11 (ExceptT String)) e1 t11) e t2) m a
...
fsubst'solve ::
  ...
  -> CS
       ...
       (ExtB
          (ExtB
             (ExtB
                (ExtB l e (StateT (Subst v (Term a cst var)))) e1 (ExceptT String))
             e11
   ...
   do u <- inc3 $ substValue v
      case u of
       Nothing -> do let fcset = FC.frsVarSet v ctxtp
                     ...
                     t' <- localSetFreshS (Set.map (True,) set)
